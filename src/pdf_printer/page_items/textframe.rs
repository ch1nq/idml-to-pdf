...
use crate::pdf_printer::color_manager;
...
impl TextFrame {
    pub fn render_story(&self,
        ...
        if let Some(story_id) = &self.parent_story() {
            if let Some(story) = idml_package.story_from_id(story_id) {
                ...
                &self.render_paragraph_style(story, &layer, idml_package.resources(), parent_transform, pdf_doc);
    ...
    pub fn render_paragraph_style(&self, 
        story: &Story, 
        ...
        match story.paragraph_style_ranges() {
            Some(p_styles) => {
                for p_style in p_styles { 
                    ...
                    &self.render_character_style(p_style, layer, idml_resources, parent_transform, pdf_doc);
                }
            },
            None => {}
        }
    ...
    pub fn render_character_style(&self, 
        ...
        match paragraph_style.character_style_ranges() {
            Some(c_styles) => {
                for c_style in c_styles {
                    ...
                    &self.render_story_content(c_style, layer, idml_resources, parent_transform, pdf_doc);
                } 
            },
            None => {}
        }
    ...
    pub fn render_story_content(&self, 
                ...
                for content in contents {
                    match content {
                        StoryContent::Content(text) => {
                            ...
                            // Get leftmost x 
                            let &(x_min, _) = points.iter().min_by(|(x1, _), (x2, _)| 
                                x1.partial_cmp(&x2).unwrap()
                            ).unwrap();
                            
                            // Get Uppermost y
                            let &(_, y_min) = points.iter().min_by(|(_, y1), (_, y2)| 
                                y1.partial_cmp(&y2).unwrap()
                            ).unwrap();

                            layer.set_text_cursor(x_min, y_min);
                            layer.set_line_height(10);
